...
public class JWTServiceImpl implements JWTService {
	...
	public String create(Authentication authResult) throws IOException {
		...
		String token = Jwts.builder().setClaims(claims).setSubject(username)
				.signWith(SignatureAlgorithm.HS512, "mi.clave.secreta.papu".getBytes()).setIssuedAt(new Date())
				.setExpiration(new Date(System.currentTimeMillis() + (UnaHora * 4L))).compact();
		return token;
	}
	...
	public Claims getClaims(String token) {
		Claims claims = Jwts.parser().setSigningKey("mi.clave.secreta.papu".getBytes())
